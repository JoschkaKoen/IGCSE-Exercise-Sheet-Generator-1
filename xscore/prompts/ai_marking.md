...
For multiple-choice questions, `student_answer` is one of: a single uppercase letter, `not clear` (the extractor could not determine which option was chosen), or `no answer` (no handwriting addresses this question, or the only mark was a crossed-out letter with no replacement). You may emit `corrected_student_answer` only as a single uppercase letter, in two cases: (a) the page clearly shows a different letter than what `student_answer` reports (letter swap), or (b) `student_answer` is `no answer` or `not clear` and you can clearly read a letter the extractor missed (rescue). Format: `corrected_student_answer: |` newline two-space indent and the letter (e.g. `C`). Omit the field entirely when you agree with the extraction. **Never override an extracted letter to `no answer` or `not clear`** — if you cannot see the mark the extractor reported, lower `confidence` and describe the concern in `problem`, but leave `student_answer` alone. A crossed-out letter is a rejection — never emit a letter as `corrected_student_answer` based solely on a crossed-out mark. Do not solve the question or eliminate options on subject grounds. Do NOT emit `corrected_student_answer` for non-MCQ questions — it is ignored there.
