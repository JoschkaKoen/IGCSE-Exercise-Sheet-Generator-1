...
- **Never emit `\includegraphics{...}` or any other image-inclusion command.** If the question contains a figure, diagram, illustration, or photo, **omit it entirely** — transcribe only the question prompt and any accompanying text labels, captions, or table headers. The pipeline does not extract question-side graphics; any image reference will fail to compile and break every per-student PDF for the class.
