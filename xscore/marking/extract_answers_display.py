...
def build_display_entries(
...
) -> tuple[list[dict], dict[tuple[str, int], int], int, int, int]:
    ...
    by (student_name, p_label).

    Returns ``(entries, idx_by_key, total_pdf_pages, n_cover, n_no_handwriting)``.
    ``total_pdf_pages`` is computed from the *full* register (every student),
    so the "/N" denominator is stable when the caller filters to a subset.
    """
    ...
    entries.sort(key=lambda e: (e["student_name"], e["p_label"]))
    idx_by_key = {(e["student_name"], e["p_label"]): i for i, e in enumerate(entries)}
    return entries, idx_by_key, total_pdf_pages, n_cover, n_no_hw
...
def emit_skipped_lines(
    ...
    for e in display_entries:
        ...
        if e["status"] == "cover":
            line = (
                f"[dim]  {icon_fn('info')}  Student '{e['student_name']}'"
                f"  ·  page {e['scan_page']:>3}/{total_pdf_pages}"
                f"  ·  cover page (skipped)[/]"
            )
        else:  # no_handwriting
            line = (
                f"[dim]  {icon_fn('info')}  Student '{e['student_name']}'"
