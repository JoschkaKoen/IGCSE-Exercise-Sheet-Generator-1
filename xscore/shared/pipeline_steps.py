...
STEPS: tuple[Step, ...] = (
    ...
    Step(19, "detect_cross_page_figures",  resumable=True,
         writes=("19_detect_cross_page_figures/*",),
         title="Detect cross-page figures"),
    Step(20, "detect_mark_scheme_graphics",writes=("20_detect_mark_scheme_graphics/*",),
         title="Detect mark scheme graphics"),
    Step(21, "assign_scheme_questions",    writes=("21_assign_scheme_questions/*",),
         title="Assign questions to mark scheme pages"),
    Step(22, "parse_mark_scheme",          writes=("22_parse_mark_scheme/*",),
         title="Parse mark scheme"),
    Step(23, "create_report",              writes=("23_create_report/*",),
         title="Build grading scaffold"),
    Step(24, "ai_marking_blueprints",      resumable=True,
         writes=("24_ai_marking_blueprints/*",),
         title="Build AI marking blueprints", section="AI marking"),
    Step(25, "ai_marking",                 resumable=True,
         writes=("25_ai_marking/*",),
         title="Run AI marking"),
    Step(26, "per_student_reports",        resumable=True,
         writes=("26_student_report_preparation/*",),
         title="Fuse AI marking output to student reports", section="Reports & PDFs"),
    Step(27, "class_stats_curve",          resumable=True,
         writes=("27_class_stats/*",),
         title="Compute class statistics + curve"),
    Step(28, "per_student_pdfs",           resumable=True,
         writes=("28_student_pdfs/*",),
         title="Generate per-student reports (landscape + portrait + 2UP)"),
    Step(29, "class_report",               resumable=True,
         writes=("29_class_report/*",),
         title="Generate class report"),
    Step(30, "review_queue",               resumable=True,
         writes=("30_review_queue/*",),
         title="Build review queue"),
    Step(31, "timing_summary",             writes=("31_timing_summary/*",),
         title="Summarise step timings", section="Summary"),
    Step(32, "accuracy_evaluation",        writes=("32_accuracy/*",),
         title="Evaluate marking accuracy"),
    Step(33, "ai_costs",                   writes=("33_ai_costs/*",),
...
def wire_step_fns() -> None:
    ...
    phase_specs: tuple[tuple[str, dict[str, str]], ...] = (
        ...
        ("xscore.steps.scaffold", {
            ...
            "detect_cross_page_figures":    "step_19_detect_cross_page_figures",
