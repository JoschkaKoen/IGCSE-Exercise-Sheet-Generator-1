...
Per-syllabus-topic markdown handouts live under `output/eXam/handouts/<subject>/<NN>.md`, with a sidecar `<NN>.meta.yaml` listing the question IDs considered when the handout was drafted. **Handouts are human/Claude-authored, not API-generated** — the cost and drift of running 100+ Qwen calls per topic outweighed the benefit when one careful writing pass produces a better handout. `web/handouts_collect.py` is the data-gathering utility: `collect_questions_for_topic(subject, topic_num)` walks the bank's `subtopic_matches.yaml` + `exam_questions.yaml`, returns `QuestionEntry` list (sorted, parent stems pre-concatenated). `load_syllabus_content_for_topic(subject, topic)` concatenates every `syllabi/content/<subject>/<N.M>.md` under the topic. The author (Claude in implementation sessions, or the human) reads both, writes the handout staying strictly within syllabus scope. Writing rules: `eXam/prompts/handout_topic.md` (kept as the authoritative style guide — markdown only, `##`/`###` headings, `$…$` math, no images/tables/hyperlinks). For incremental updates (e.g. when a new paper gets indexed and matched), use the Edit tool to make line-level changes rather than rewriting the file. Markdown → HTML and markdown → LaTeX/PDF rendering arrive in follow-up PRs.
