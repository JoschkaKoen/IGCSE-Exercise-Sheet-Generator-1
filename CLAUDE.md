...
**Lesson content** is committed authored source under `content/code/<course>/` (NOT `output/`): a `course.yaml` manifest plus per-lesson `NN.en.md` / `NN.zh.md` (bilingual prose) and `NN.meta.yaml` (title + task list — each task has `starter` code, an optional scripted `stdin`, and a client-side `check` spec — `{kind: stdout, …}`, `{kind: asserts, …}` (Python), `{kind: harness, …}` (Java/C), or `{kind: rows, …}` (SQL)). Loaded by `web/code_content.py` at request time (no in-process cache, like the Learn page); this mirrors the handouts authored-markdown-plus-YAML-sidecar pattern. Six courses today: `python-basics` (15 lessons), `a-level-cs` (15, Python), `ap-csp` (15, Python), `ap-csa` (19, Java), `c-basics` (23, C, server-side), and `sql-databases` (17, SQL, client-side via sql.js). Validate authored lessons with `python -m scripts.check_code_lessons`.
...
**SQL runner (client-side via sql.js).** The `sql-databases` course runs **entirely in the browser**: `web/static/js/code-worker-sql.js` is a classic worker that `importScripts` vendored sql.js (SQLite-in-WASM), opens a fresh in-memory DB per run, applies the task's seed, runs the student's SQL, and renders result grids to canonical text. There is **no server endpoint** (unlike Java/C). `web/sql_runner.py` is the pure stdlib-`sqlite3` core — the **same SQLite engine** — imported by `scripts/check_code_lessons.py`, giving the same **validator-pass ⇒ runtime-pass** guarantee; its `render_grid` is kept byte-identical to the worker's `renderGrid` (floats normalised — author averages with `ROUND(x, 2)`). The `rows` check compares the rendered grid to `expected`, with an optional `probe` SELECT that reads DB state after INSERT/UPDATE/DELETE/CREATE (DDL type checks normalise via `UPPER(REPLACE(type,' ',''))`). Seeds live in `content/code/sql-databases/datasets/*.sql`, resolved by `code_content.resolve_seed` (lesson-level `dataset:`/`seed:`, per-task override). sql.js + the CodeMirror SQL mode are vendored via `scripts/vendor_fetch.py`; the per-course landing accent is set in `web/static/css/16-code.css`.
